...
3. `run.sh` tự động thực hiện đúng thứ tự:
   ...
   - start JAR mới để Flyway baseline `0` và chạy V1/V2 hiện có;
...
4. Flyway sẽ:
   ...
   - chạy V1 tạo các bảng `hr_*`, sau đó V2 bổ sung retention staging;
...
6. Các lần chạy sau dùng launcher/`build-prod.sh` bình thường. Flyway đọc history hiện có và V1/V2 sẽ là no-op.
...
- Nếu V1/V2/verifier lỗi: backend mới bị dừng, tunnel vẫn đóng; giữ log và final backup, không tự chạy `repair` hoặc drop table từng phần.
...
## 8. Gate Phase 1 (đã đạt trước Phase 2)

- Chưa seed/import 329 nhân sự trong V1/V2.
...
- Parser/import transaction/rollback/retention Phase 2 đã pass trên MySQL 8 cô lập. Schema hiện tại có thêm V2 retention, không sửa bảng legacy; xem [HR Phase 2](HR_PHASE_2_BASELINE_IMPORT.md).
