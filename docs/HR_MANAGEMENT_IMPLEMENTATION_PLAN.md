...
Cập nhật: 2026-07-24
Trạng thái: **Phase 0.1–6 hoàn thành ở source code và automated test; Phase 5–6 chờ deploy/UAT runtime; Phase 7 ngày phép tự động đã gỡ/defer**
...
### Phase 7 — Ngày phép tự động

Trạng thái: **đã gỡ/defer ngày 2026-07-24**.

- Đã gỡ phần tính ngày phép tự động vì công thức nghiệp vụ chưa chốt.
- Không còn `HrLeaveEntitlementService`, API `currentLeaveDays` hoặc UI hiển thị ngày phép tự tính.
- Không xóa schema/database `leave_days` để tránh mất dữ liệu import/snapshot.
- Nếu làm lại, nên tạo phase riêng với rule bằng văn bản từ TCHC, version/effective date, case test và audit điều chỉnh.
...
Phase 7 hiện không còn active:

- Tính ngày phép tự động đã gỡ khỏi backend/frontend.
- Cột/schema `leave_days` vẫn giữ để không mất dữ liệu đã import hoặc snapshot cũ.
- Không tiếp tục phase này nếu chưa có công thức nghiệp vụ chính thức.
