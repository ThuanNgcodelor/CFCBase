...
export const hrPayrollApi = {
  ...
  refreshEligibility: async (id) => unwrapApiData(await baseApi.post(`/hr/payroll/imports/${id}/refresh-eligibility`)),
  createCampaign: async (id, selectionMode = 'ALL_ELIGIBLE', rowIds = []) => unwrapApiData(await baseApi.post(`/hr/payroll/imports/${id}/campaigns`, { deliveryMode: 'PDF', selectionMode, rowIds })),
  ...
  resend: async (campaignId, deliveryId, reason) => unwrapApiData(await baseApi.post(`/hr/payroll/campaigns/${campaignId}/deliveries/${deliveryId}/resend`, { reason })),
  ...
  document: async (campaignId, deliveryId) => (await baseApi.get(`/hr/payroll/campaigns/${campaignId}/deliveries/${deliveryId}/document`, { responseType: 'blob' })).data,
  testRecipient: async (options = {}) => unwrapApiData(await baseApi.get('/hr/payroll/test-recipient', { signal: options.signal, _silent: true })),
  createTestRecipientLink: async () => unwrapApiData(await baseApi.post('/hr/payroll/test-recipient/link')),
  revokeTestRecipient: async () => unwrapApiData(await baseApi.delete('/hr/payroll/test-recipient')),
  sendTest: async (importId, rowId) => unwrapApiData(await baseApi.post(`/hr/payroll/imports/${importId}/rows/${rowId}/test-send`)),
};
