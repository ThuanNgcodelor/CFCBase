...
function AttendanceTabs() {
  ...
  return <div className="mb-5 inline-flex rounded-xl border border-gray-200 bg-white p-1 shadow-sm">
    ...
    <button type="button" onClick={() => navigate('/manager/hr/attendance/night-rewards')} className="rounded-lg px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-50">Thưởng ca đêm</button>
  </div>;
}
...
function ConfigurationDrawer({ open, policies, creditRules, onClose, onRefresh }) {
  ...
  const [assignment, setAssignment] = useState({ policyGroup: 'PRODUCTION_WORKER', dayWorkValueOverride: '', validFrom: currentMonth() + '-01', validTo: '', reason: '' });
  ...
  const savePolicy = async (item, form) => {
    try { await api.updateShiftPolicy(item.id, { name: form.name, standardStart: form.standardStart, standardEnd: form.standardEnd, checkInFrom: form.checkInFrom, checkInUntil: form.checkInUntil, checkOutFrom: form.checkOutFrom, checkOutUntil: form.checkOutUntil, nightAllowanceAmount: Number(form.nightAllowanceAmount), countsTowardNightReward: Boolean(form.countsTowardNightReward), priority: Number(form.priority), active: form.active, validFrom: form.validFrom, validTo: form.validTo || null, rowVersion: item.rowVersion }); toast.success('Đã lưu cấu hình ca.'); onRefresh(); }
  ...
  const createAssignment = async (event) => {
    event.preventDefault();
    try { await api.createEmployeePolicy({ employeeCode: employeeCode.trim(), ...assignment, dayWorkValueOverride: assignment.dayWorkValueOverride ? Number(assignment.dayWorkValueOverride) : null, validTo: assignment.validTo || null }); toast.success('Đã gán chính sách chấm công. Bấm Tính lại trên file để áp dụng.'); setAssignment({ ...assignment, reason: '' }); lookup(); onRefresh(); }
  ...
  return <HrDrawer isOpen={open} onClose={onClose} title="Cấu hình ca sản xuất" description="Ca, nhóm chính sách theo thời gian và miễn chấm được quản lý độc lập." size="wide">
    <div className="space-y-6 p-5 sm:p-7">
      ...
      <section className="border-t border-gray-200 pt-5"><h3 className="font-semibold text-gray-900">Nhân viên và miễn chấm</h3><div className="mt-3 flex gap-2"><input value={employeeCode} onChange={(event) => setEmployeeCode(event.target.value.toUpperCase())} placeholder="Mã nhân viên" className="h-11 min-w-0 flex-1 rounded-lg border border-gray-300 px-3" /><Button type="button" variant="secondary" onClick={lookup}>Tra cứu</Button></div>
        <form onSubmit={createAssignment} className="mt-4 grid gap-3 rounded-xl bg-slate-50 p-4 sm:grid-cols-2"><p className="font-semibold sm:col-span-2">Gán chính sách chấm công</p><select value={assignment.policyGroup} onChange={(event) => setAssignment({ ...assignment, policyGroup: event.target.value })} className="h-11 rounded-lg border border-gray-300 bg-white px-3"><option value="PRODUCTION_WORKER">Công nhân</option><option value="KCS">KCS</option><option value="OFFICE">Hành chính</option></select><select value={assignment.dayWorkValueOverride} onChange={(event) => setAssignment({ ...assignment, dayWorkValueOverride: event.target.value })} className="h-11 rounded-lg border border-gray-300 bg-white px-3"><option value="">Tính theo mốc giờ ra</option><option value="1.5">Ca ngày cố định 1,5 công</option><option value="1">Ca ngày cố định 1 công</option><option value="2">Ca ngày cố định 2 công</option></select><input value={assignment.reason} onChange={(event) => setAssignment({ ...assignment, reason: event.target.value })} required placeholder="Lý do" className="h-11 rounded-lg border border-gray-300 px-3 sm:col-span-2" /><input type="date" value={assignment.validFrom} onChange={(event) => setAssignment({ ...assignment, validFrom: event.target.value })} required className="h-11 rounded-lg border border-gray-300 px-3" /><input type="date" value={assignment.validTo} onChange={(event) => setAssignment({ ...assignment, validTo: event.target.value })} className="h-11 rounded-lg border border-gray-300 px-3" /><p className="text-xs text-gray-500 sm:col-span-2">Mức cố định chỉ áp dụng khi ghép đủ lượt vào/ra của ca ngày; ca đêm và ngày thiếu dấu vẫn xử lý riêng.</p><Button type="submit" className="sm:col-span-2">Lưu chính sách</Button></form>
        <div className="mt-3 space-y-2">{employeePolicies.map((item) => <div key={item.id} className="rounded-lg border border-gray-200 p-3 text-sm"><b>{policyLabel(item.policyGroup)}</b>{item.dayWorkValueOverride ? ` · Ca ngày cố định ${work(item.dayWorkValueOverride)} công` : ' · Theo mốc giờ ra'} · {formatHrDate(item.validFrom)} → {formatHrDate(item.validTo)}<p className="text-gray-500">{item.reason}</p></div>)}</div>
