...
export default function HrPayroll() {
  ...
  const deleteImport = async (event, item) => {
    ...
    if (!window.confirm(`Xóa bản xem trước "${item.fileName}"? Thao tác này không thể hoàn tác.`)) return;
    setBusy(true);
    try {
  ...
  return <HrPageShell size="wide">
    ...
    {loading ? <div className="mt-5"><HrLoading /></div> : error ? <div className="mt-5"><HrError message={error} onRetry={loadImports} /></div> : <section className="mt-5 rounded-xl border border-gray-200 bg-white shadow-sm">
      <div className="border-b border-gray-100 p-4"><h2 className="font-semibold text-gray-900">2. Chọn file đã import</h2><p className="text-sm text-gray-500">File nháp chưa tạo đợt gửi có thể xóa. File đã gửi được giữ để đối soát.</p></div>
      <div className="overflow-x-auto"><table className="min-w-[950px] w-full text-left text-sm"><thead className="bg-gray-50 text-xs uppercase tracking-wide text-gray-500"><tr><th className="px-4 py-3">File</th><th>Tháng</th><th>Tổng dòng</th><th>Sẵn sàng</th><th>Chưa đủ</th><th>Trạng thái</th><th>Thời gian</th><th className="px-4 text-right">Thao tác</th></tr></thead><tbody className="divide-y">{imports.content.map(item => <tr key={item.id} onClick={() => { setSelectedRows(new Set()); selectImport(item); }} className={`cursor-pointer hover:bg-emerald-50 ${selected?.id === item.id ? 'bg-emerald-50' : ''}`}><td className="px-4 py-3 font-medium">{item.fileName}</td><td>{item.payrollMonth || '—'}</td><td>{item.totalRows}</td><td className="text-emerald-700">{item.readyRows}</td><td className="text-amber-700">{item.skippedRows}</td><td><HrStatusBadge status={item.status} label={statusLabel(item.status)} /></td><td className="text-xs text-gray-500">{formatHrDateTime(item.createdAt)}</td><td className="px-4 text-right"><Button type="button" variant="danger" size="icon" disabled={busy || item.status !== 'PREVIEWED'} title={item.status === 'PREVIEWED' ? 'Xóa file nháp' : 'File đã vào quy trình gửi nên chỉ lưu lịch sử'} onClick={event => deleteImport(event, item)}><Trash2 className="h-4 w-4" /></Button></td></tr>)}</tbody></table></div>
