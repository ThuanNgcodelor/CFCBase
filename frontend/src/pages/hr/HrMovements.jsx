  ...
  return (
    <HrPageShell>
      ...
      <div className="hidden overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm md:block">
        <table className="w-full min-w-[1120px] divide-y divide-gray-200"><thead className="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wider text-gray-500"><tr><th className="px-5 py-4">Nhân sự</th><th className="px-5 py-4">Loại</th><th className="px-5 py-4">Hiệu lực</th><th className="px-5 py-4">Thay đổi</th><th className="px-5 py-4">Trạng thái</th><th className="px-5 py-4">Người xử lý</th><th className="px-5 py-4">Thao tác</th></tr></thead>
          <tbody className="divide-y divide-gray-100">{loading ? <tr><td colSpan="7" className="px-5 py-12 text-center text-sm text-gray-500">Đang tải...</td></tr> : result.content.map((item) => <tr key={item.id} className="align-top hover:bg-gray-50/70"><td className="px-5 py-4"><p className="text-sm font-medium text-gray-900">{item.employeeName || item.fullName}</p><p className="mt-1 text-xs text-gray-500">{item.employeeCode}</p></td><td className="whitespace-nowrap px-5 py-4 text-sm text-gray-700">{movementLabel(item.movementType || item.type)}</td><td className="whitespace-nowrap px-5 py-4 text-sm text-gray-600">{formatHrDate(item.effectiveDate)}</td><td className="px-5 py-4"><MovementRoute item={item} />{item.reason && <p className="mt-2 max-w-sm text-xs text-gray-500">{item.reason}</p>}{item.decisionNumber && <p className="mt-1 text-xs text-gray-500">QĐ: {item.decisionNumber} · {formatHrDate(item.decisionDate)}</p>}</td><td className="whitespace-nowrap px-5 py-4"><HrStatusBadge status={item.status} /></td><td className="px-5 py-4"><p className="max-w-[180px] truncate text-xs font-medium text-gray-700">{displayActor(item.confirmedByActor || item.cancelledByActor || item.createdByActor)}</p><p className="mt-1 whitespace-nowrap text-xs text-gray-400">{formatHrDateTime(item.confirmedAt || item.cancelledAt || item.createdAt)}</p></td><td className="px-5 py-4"><ActionButtons item={item} /></td></tr>)}{!loading && result.content.length === 0 && <tr><td colSpan="7" className="p-5"><HrEmpty title="Chưa có biến động phù hợp" /></td></tr>}</tbody>
        </table>
      </div>
      <div className="space-y-3 md:hidden">{loading ? <div className="rounded-xl border bg-white py-10 text-center text-sm text-gray-500">Đang tải...</div> : result.content.map((item) => <div key={item.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><p className="font-medium text-gray-900">{item.employeeName || item.fullName}</p><p className="mt-1 text-xs text-gray-500">{item.employeeCode} · {formatHrDate(item.effectiveDate)}</p></div><HrStatusBadge status={item.status} /></div><p className="mt-3 text-sm font-medium text-emerald-700">{movementLabel(item.movementType || item.type)}</p><div className="mt-2"><MovementRoute item={item} /></div>{item.reason && <p className="mt-2 text-xs text-gray-500">{item.reason}</p>}{item.decisionNumber && <p className="mt-1 text-xs text-gray-500">QĐ: {item.decisionNumber} · {formatHrDate(item.decisionDate)}</p>}<p className="mt-3 text-xs text-gray-400">{displayActor(item.confirmedByActor || item.cancelledByActor || item.createdByActor)} · {formatHrDateTime(item.confirmedAt || item.cancelledAt || item.createdAt)}</p><div className="mt-3"><ActionButtons item={item} /></div></div>)}{!loading && result.content.length === 0 && <HrEmpty title="Chưa có biến động phù hợp" />}</div>
